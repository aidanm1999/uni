...
else if (substr($queryArray[0], 0) != "@")
{
	$untaggedQuery = [];
	$queryStringPrep = "SELECT * FROM stats WHERE (";
	for ($x = 0; $x < count($queryArray); $x++) { //Element in query array
		$queryStringPrep=$queryStringPrep."title like '%".$queryArray[$x]."%' or label like '%".$queryArray[$x]."%'";
		if($x=count($queryArray))
		{
			//Last item needs to close bracket
			$queryStringPrep=$queryStringPrep.");";
		}
	...
	$untaggedQuery=$conn->prepare($queryStringPrep);
	$untaggedQuery->execute();
	$untaggedResults=$untaggedQuery->fetchAll(PDO::FETCH_ASSOC);
...
<html>
	...
	<head>
		...
		<title>Search: <?php echo $term;?> - U+Ni</title>
	...
	<script>
		...
		var untaggedStatisticsResults = <?php echo json_encode($untaggedResults)?>;
		...
		$(function() {
			...
			//No labels have been selected - Search all APIS
			else
			{
				//Untagged results = results array from apis
				//jsQueryArray = input from user
				
				var untaggedUniversitiesResults = [];
				var untaggedJobsResults = [];
				var jsQueryArray = <?php echo json_encode($queryArray); ?>;
				//Statistics have already been found
				...
				//DISPLAY RESULTS
				untaggedStatisticsResults.forEach(function(result) {

					var dates = [];
					var values = [];
					$.ajax({															//CHANGE BACK TO STATISTIC
						url: "http://api.worldbank.org/v2/countries/GBR/indicators/"+result.code+"?date=2009:2015&format=json",
						type: "get",
						dataType: "json", //Data[1] filters out rubbish data
						success: function(data) {
